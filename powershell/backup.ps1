param(  $serverName,$databaseName,$backupDirectory,$SQLLogin,$SQLPassword)
...
function ConnectToDatabase($server, $SqlLogin, $SqlPassword)
{
...
function AddPercentHandler($smoBackupRestore, $action) {
...
function CreatDevice($smoBackupRestore, $directory, $name) {
$devicePath = [System.IO.Path]::Combine($directory, $name)
$smoBackupRestore.Devices.AddDevice($devicePath, "File")
return $devicePath
}
function CreateDevices($smoBackupRestore, $devices, $directory, $dbName, $incremental) {
...
function BackupDatabase($dbName, $devices, $compressionOption, $incremental, $copyonly) {
...
if ($database -eq $null) {
Write-host "Database $DatabaseName does not exist on $ServerName"
Exit
}
if ($Incremental -eq $true) {
